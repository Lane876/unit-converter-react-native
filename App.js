...
const MeasureView = ({measure, value, setValue})=> {
  ...
  useEffect(()=>{
    setValueConverter(convert(+value).from(fromUnit).to(toUnit).toFixed(2))
...
export default function App() {
  ...
  return (
    <View style={[styles.scene, {marginTop: Constants.statusBarHeight}]}>
      ...
      renderTabBar={(props)=><TabBar {...props} scrollEnabled tabStyle={{width:'auto'}} indicatorStyle={{backgroundColor:'white'}} style={{backgroundColor: mainColor}}/>}    
      />
    </View>
  );
}
